...
def connect_to_server(threadName, delay):
    try:
        ...
        data = s.recv(1024)
        s_print("Local Thread " + threadName + ": " + data.decode())
        receive_lock.release()

    except socket.error as ex:
        template = (
            "An exception of type in thread "
            + threadName
            + " {0} occurred. Arguments:\n{1!r}"
        )
...
print(
    "Connecting to {ipadd} on port {prt}, with {noofthreads} threads, each sending {noPackets} packets!".format(
