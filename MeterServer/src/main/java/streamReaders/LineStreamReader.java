...
public class LineStreamReader extends StreamReader{
	...
	public Boolean parse() throws IOException {
		...
		Integer lNumberOfNewBytes;
		StringBuilder sb = new StringBuilder();
		while(!finsihedFlag) {

			lNumberOfNewBytes = fInputStream.read(lBuffer);
			//there is no new fresh bytes to read, and we haven't exited correctly, then there has been
			//some interuption in the transmition that has caused an abnormal end.
			//do not push the data into the queue, just log an error 
			//TODO maybe throw an exception too.
			if(!(lNumberOfNewBytes > 0)) {
				fLogger.error("LineStreamReader has abnormally ended.");
				return false;
			}
				//read the byte stream and look for new lines. if we find a new line character,
				//then we split the string that contains the bytes since the last new line,
				//into its invidiual values, parse them  and then push it into the Lines list, which will
				//be pushed into the Dataqueue for Database Workers to process.
				for(byte b: lBuffer) {
					if((char)b == '\n' || (char)b == '\r') {
					...
					else if(b == 33 || b ==0) { //Exclamation mark received
						finsihedFlag = true;
						break;
					} else {
						sb.append((char)b);
